...
public class CombatTagCmd {
    ...
    public static void onCommand(@NotNull CommandArgs args) {
        ...
        if (args.length() != 0) {
            MessageUtil.messagePlayer(sender, "&cUsage: /combattag");
            return;
        }

        // Checks if the combat tag feature is enabled.
        if (!Settings.combatTagEnabled) {
            MessageUtil.messagePlayer(sender, ConstantUtil.COMMAND_DISABLED);
            return;
        }
        ...
        if (CombatTag.isInCombat(player)) {
            ...
            MessageUtil.messagePlayer(sender, "&cYou are in combat for " + timeLeft
                    + " more " + (timeLeft == 1 ? "second" : "seconds") + ".");
        } else {
            MessageUtil.messagePlayer(sender, "&aYou are not in combat.");
        }
    }
}
